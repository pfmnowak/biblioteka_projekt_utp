					...
					if(isset($_SESSION['id'])) {
						...
						try {
							...
							if($polaczenie->connect_errno!=0) {
								...
								$moje_id = $_SESSION['id'];
								$rezultat = $polaczenie->query("SELECT w.data_zwrotu, e.id, k.tytul, k.rok_wydania, k.strony, a.nazwisko, w.data_wypozyczenia, w.oddana FROM wypozyczenie AS w, egzemplarz AS e, ksiazka AS k, autor AS a WHERE w.idc = '$moje_id' AND w.ide = e.id AND e.idk = k.id AND k.ida = a.id ORDER BY w.data_wypozyczenia DESC");
								if(!$rezultat) throw new Exception($polaczenie->error);
								$ile_wypozyczen = $rezultat->num_rows;
								if($ile_wypozyczen>0) {
									echo '<table class="results"><tr><th>Nr egz.</th><th>Autor</th><th>Tytuł</th><th>Rok wydania</th><th>Liczba stron</th><th>Data wypożyczenia</th><th>Data zwrotu</th></tr>';
									while ($wiersz = $rezultat->fetch_array()) {
										...
										$strony = $wiersz[4];
										$nazwisko = $wiersz[5];
										$data_wypozyczenia = $wiersz[6];
										$oddana = $wiersz[7];
										$zwrot = $data_zwrotu;
										if($oddana == 0) {
											$zwrot = 'Wypożyczona';
										}
										echo '<tr><td>'.$ide.'</td><td>'.$tytul.'</td><td>'.$nazwisko.'</td><td>'.$rok_wydania.'</td><td>'.$strony.'</td><td>'.$data_wypozyczenia.'</td><td>'.$zwrot.'</td></tr>';
									}
									echo '</table>';
								} else {

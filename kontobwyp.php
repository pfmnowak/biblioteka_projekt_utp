...
<body>
	...
	<div class="container">
		...
		<article>
			<div class="content">
				<h3>Wszystkie obecnie wypożyczone książki</h3>
				<?php
					if(isset($_SESSION['id_bibl'])) {
						...
						try {
							...
							if($polaczenie->connect_errno!=0) {
								...
								$rezultat = $polaczenie->query("SELECT w.data_zwrotu, e.id, k.tytul, k.rok_wydania, a.nazwisko, w.data_wypozyczenia, w.idc FROM wypozyczenie AS w, egzemplarz AS e, ksiazka AS k, autor AS a WHERE w.oddana = '0' AND w.ide = e.id AND e.idk = k.id AND k.ida = a.id ORDER BY w.data_wypozyczenia DESC");
								...
								if($ile_wypozyczen>0) {
									...
									while ($wiersz = $rezultat->fetch_array()) {
										...
										$czyt = $wiersz[6];
										echo '<tr><td>'.$ide.'</td><td>'.$nazwisko.'</td><td>'.$tytul.'</td><td>'.$rok_wydania.'</td><td>'.$data_wypozyczenia.'</td><td>'.$data_zwrotu.'</td><td>'.$czyt.'</td></tr>';
									}
									echo '</table>';
								} else {
